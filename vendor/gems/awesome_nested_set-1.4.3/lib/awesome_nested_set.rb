    ...
    module NestedSet #:nodoc:
      ...
      module ClassMethods
        ...
        def rebuild!
          ...
          puts "Building some weird sql thing."
          scope = lambda{|node|}
          if acts_as_nested_set_options[:scope]
            scope = lambda{|node| 
          ...
          puts "Set left and right nodes"
          set_left_and_rights = lambda do |node|
            # set left
            node[left_column_name] = indices[scope.call(node)] += 1
            # find
            find(:all, :conditions => ["#{quoted_parent_column_name} = ? #{scope.call(node)}", node], :order => "#{quoted_left_column_name}, #{quoted_right_column_name}, id").each do |n|
               set_left_and_rights.call(n)
             end
            # set right

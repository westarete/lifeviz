/*
 * So here's the deal. jsTree does everything we need it to, except for one thing
 * that I just can't make work only because of the structure of its HTML output. 
 *
 * The sane answer is to write a jsTree plugin that would override the one function
 * that creates new nodes in the dom--create_node--to do what I need. 
 *
 * The problem is, the JSON Data plugin does not use this method. It duplicates the
 * dom creation code inside its own (messy) _parse_json method, making it impossible
 * to hook it with a plugin. Nice design, jsTree!
 *
 * The two options are to refactor the JSON Data plugin to hit the create_node function,
 * or just hack the code. I'm lazy, and jsTree's shitty architecture is not my problem,
 * so I opted to hack the code.
 *
 * Ryan Schenk 2010
 */
	...
	$.jstree.plugin("core", {
		...
		_fn : { 
			init	: function () { 
				...
				this.get_container()
					.delegate("li .term > ins", "click.jstree", $.proxy(function (event) {
		...
		_fn : {
			...
			_parse_json : function (js, is_callback) {
				...
				if($.isArray(js)) {
					...
					term.prepend("<ins class='jstree-icon'>&#160;</ins>");
					d.append(term);
					if(js.children) { 
	...
	$.jstree.plugin("themeroller", {
		...
		_fn : {
			_themeroller : function (obj) {
				...
				obj
					.find("li.jstree-closed .term > ins.jstree-icon").removeClass(s.opened).addClass("ui-icon " + s.closed).end()
					.find("li.jstree-open .term > ins.jstree-icon").removeClass(s.closed).addClass("ui-icon " + s.opened).end()
